        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className={`px-3 py-2 rounded-lg text-sm flex items-center gap-2 ${
              autoRefresh ? 'btn-gradient-success' : 'bg-slate-700 text-slate-400'
            }`}
          >
            <RefreshCw className={`w-4 h-4 ${autoRefresh ? 'animate-spin' : ''}`} />
            Auto
          </Button>
          <Button onClick={fetchData} className="bg-slate-700 hover:bg-slate-600 text-white flex items-center gap-2">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.sessions.map(session => (
                    <tr key={session.id} className="border-t border-zinc-800 hover:bg-zinc-800/50 transition-colors">
