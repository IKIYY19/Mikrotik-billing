...
function StatCard({ icon: Icon, label, value, color, onClick, active }) {
  return (
    <button
      onClick={onClick}
      className={`flex-1 min-w-[80px] flex flex-col items-center gap-1 rounded-xl p-2.5 border transition-all duration-200 cursor-pointer
        ${active
    ...
    >
      ...
      <span className={`text-lg font-bold ${active ? color.text : "text-white"}`}>{value}</span>
...
function CustomerListItem({ customer, selected, onClick }) {
  ...
  return (
    ...
    >
      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${c.text.replace("text-", "bg-")}`}
      ...
      {!customer.lat && (
        <MapPin className="w-3 h-3 text-slate-600 flex-shrink-0" title="No location set" />
      )}
...
export function MapView() {
  ...
  const markersRef = useRef([]);         // Customer & branch markers
  const clickMarkerRef = useRef(null);   // Temporary click marker
  const leafletRef = useRef(null);       // L (Leaflet library)
  ...
  const fetchData = async () => {
    try {
      ...
      // Provide empty fallback so map still renders
      setData({ customers: [], branches: [], towers: [], center: [-1.2921, 36.8219], zoom: 10 });
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (!data || !leafletRef.current || !mapDivRef.current) return;
    if (mapRef.current) return; // already initialised

    const L = leafletRef.current;

    // Fix default marker icon paths (Vite/webpack asset bundling issue)
    ...
    // ── Click handler (coordinate picker) ─────────────────────────────────
    map.on("click", (e) => {
  ...
  const renderMarkers = useCallback(() => {
    ...
    branches.forEach((branch) => {
      ...
      marker.bindPopup(`
        <div style="font-family:system-ui;min-width:160px">
          <div style="font-weight:700;font-size:13px;margin-bottom:6px;color:#1e3a5f">🏢 ${branch.name}</div>
          <div style="font-size:11px;color:#555;line-height:1.6">
            ${branch.city ? `<div>📍 ${branch.city}</div>` : ""}
            <div>🟢 Active PPPoE: <b>${branch.active_pppoe || 0}</b></div>
            <div>📡 Routers: <b>${branch.online_routers || 0}/${branch.total_routers || 0}</b> online</div>
          </div>
        </div>`, { maxWidth: 220 });
    ...
    towers.forEach((tower) => {
      ...
      marker.bindPopup(`
        <div style="font-family:system-ui;min-width:150px">
          <div style="font-weight:700;font-size:13px;margin-bottom:6px;color:#4c1d95">📡 ${tower.name}</div>
          <div style="font-size:11px;color:#555;line-height:1.6">
            <div>👥 Clients: <b>${tower.customer_count || 0}</b></div>
            ${tower.height ? `<div>📏 Height: <b>${tower.height}m</b></div>` : ""}
          </div>
        </div>`, { maxWidth: 200 });
    ...
    withCoords.forEach((customer) => {
      ...
      const marker = L.marker([customer.lat, customer.lng], {
        icon: L.icon({
          iconUrl: makeSvgIcon(col.fill, col.ring),
          iconSize: [28, 36],
          iconAnchor: [14, 36],
          popupAnchor: [0, -38],
        }),
      ...
      marker.bindPopup(`
        <div style="font-family:system-ui;min-width:170px">
          <div style="font-weight:700;font-size:13px;margin-bottom:6px">${customer.name}</div>
          <div style="font-size:11px;color:#555;line-height:1.8">
            <div>Status: <span style="color:${col.fill};font-weight:600">${col.label}</span></div>
            <div>Plan: <b>${customer.plan || "—"}</b></div>
            <div>Phone: ${customer.phone || "—"}</div>
            <div style="margin-top:6px;font-size:10px;color:#999">${customer.lat.toFixed(5)}, ${customer.lng.toFixed(5)}</div>
          </div>
        </div>`, { maxWidth: 230 });
  ...
  // ── Fly to a customer on sidebar click ──────────────────────────────────
  ...
  const filteredList = customers.filter((c) => {
    ...
    const matchSearch = !search || c.name.toLowerCase().includes(search.toLowerCase()) ||
      (c.phone && c.phone.includes(search)) || (c.plan && c.plan.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="flex h-full bg-slate-900 overflow-hidden" style={{ minHeight: 0 }}>
      ...
      >
        ...
        <div className="p-3 border-b border-slate-800 flex gap-2 flex-shrink-0">
          ...
          <StatCard
            icon={AlertTriangle}
            label="Throttled"
            value={stats.throttled}
            color={STATUS_COLORS.throttled}
            onClick={() => setFilter(filter === "throttled" ? "all" : "throttled")}
        ...
        <div className="p-3 border-t border-slate-800 flex-shrink-0">
          <div className="flex flex-wrap gap-x-3 gap-y-1">
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2" style={{ width: 0, height: 0, borderLeft: "4px solid transparent", borderRight: "4px solid transparent", borderBottom: "8px solid #8b5cf6" }} />
      ...
      <div className="flex-1 relative" style={{ minWidth: 0 }}>
        ...
        {/* No customers with location hint */}
        {!loading && stats.withLocation === 0 && (
          <div className="absolute inset-0 flex items-center justify-center z-[400] pointer-events-none">
            <div className="bg-slate-900/90 backdrop-blur border border-slate-700 rounded-2xl p-6 text-center max-w-xs shadow-2xl pointer-events-auto">
              ...
              <h3 className="text-white font-semibold mb-1">No locations set</h3>
              <p className="text-slate-400 text-sm">
                Click anywhere on the map to get coordinates, then add them to customer profiles to see them here.
              </p>
            </div>
          </div>
        )}
