...
Three deployment options are available: **VPS Docker Compose** (recommended), **Dokploy**, or **Northflank**.
...
Northflank is a developer-friendly cloud platform that allows you to easily build, deploy, and scale containers.

### Prerequisites
1. A Northflank account.
2. Code pushed to a Git repository (GitHub, GitLab, or Bitbucket) linked to your Northflank account.

### Step 1: Create a PostgreSQL Addon on Northflank
1. In your Northflank dashboard, click **Create New** → **Addon**.
2. Select **PostgreSQL** as the database engine.
3. Choose your plan (the Hobby/Free tier is sufficient for testing).
4. Set the name to `mikrotik-db` and click **Create Addon**.
5. Once running, Northflank will automatically make the database environment variables available when you link this addon to your service.

### Step 2: Create a Combined Service (Web Service)
1. Click **Create New** → **Service** → **Combined Service**.
2. Choose **Repository** and select your repository and the target branch (e.g., `master` or `main`).
3. Under **Build Settings**:
   - Select **Dockerfile** as the build method.
   - Set the Dockerfile path to `Dockerfile.northflank`.
   - Add build arguments if needed (e.g. `VITE_API_URL` defaults to `/api`).
4. Under **Deployment**:
   - Add a port mapping exposing port `5000` (which is the default internal port exposed by `Dockerfile.northflank`). Set the protocol to `HTTP`.
5. Under **Environment Variables**:
   - Link the PostgreSQL addon created in Step 1. Northflank will automatically inject the `DATABASE_URL` and database credentials (`DB_HOST`, `DB_PORT`, etc.).
   - Add other required environment variables:
     - `NODE_ENV` = `production`
     - `JWT_SECRET` = (generate a secure random string)
     - `ENCRYPTION_KEY` = (generate a secure 32-character hexadecimal key)
6. Click **Create Service**.

### Step 3: Verify and Access
1. Once the service builds and deploys, check the container logs in Northflank. You should see:
   - Database connection verified.
   - Migrations running and completing successfully.
   - Server starting up on port `5000`.
2. Access the public URL generated by Northflank (e.g., `https://your-app-id.code.run`).
