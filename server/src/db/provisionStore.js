...
function generateProvisionScript(router) {
  ...
  lines.push(`/ip dhcp-client add interface=${wanIface} add-default-route=yes use-peer-dns=no comment="WAN - Auto-configured"`);
  lines.push('');

  // LAN Bridge
  lines.push('# LAN Bridge Configuration');
  lines.push(`/interface bridge add name=${lanBridge} protocol=rstp comment="Auto-created LAN bridge"`);
  lines.push('');

  // Add LAN ports to bridge (ether2-ether8 or all ports except WAN)
  lines.push('# Bridge LAN Ports');
  const lanPorts = router.lan_ports || ['ether2', 'ether3', 'ether4', 'ether5', 'ether6', 'ether7', 'ether8'];
  lanPorts.forEach(port => {
    if (port !== wanIface) {
      lines.push(`/interface bridge port add bridge=${lanBridge} interface=${port} comment="Auto-bridge ${port}"`);
    }
  });
  lines.push('');
  ...
  lines.push(`/ip address add address=${lanIp} interface=${lanBridge} comment="LAN Gateway"`);
  lines.push('');

  // NAT Masquerade
  lines.push('# NAT Configuration');
  lines.push(`/ip firewall nat add chain=srcnat action=masquerade out-interface=${wanIface} comment="Auto-NAT Masquerade"`);
  lines.push('');

  // DHCP Server for LAN
  lines.push('# DHCP Server Configuration');
  lines.push('/ip pool add name=dhcp_pool ranges=' + dhcpStart + '-' + dhcpEnd);
  lines.push(`/ip dhcp-server network add address=${lanNetwork} gateway=${lanGateway} dns-server=${lanGateway} domain=local`);
  lines.push(`/ip dhcp-server add name=dhcp_lan interface=${lanBridge} address-pool=dhcp_pool disabled=no lease-time=3d comment="Auto DHCP Server"`);
  lines.push('');

  // Advanced Firewall Rules
  lines.push('# Firewall Filter Rules');
  lines.push('# Input Chain');
  lines.push('/ip firewall filter add chain=input action=accept connection-state=established,related,untracked comment="Allow established connections"');
  lines.push('/ip firewall filter add chain=input action=drop connection-state=invalid comment="Drop invalid connections"');
  lines.push('/ip firewall filter add chain=input protocol=icmp action=accept comment="Allow ICMP ping"');
  lines.push('/ip firewall filter add chain=input protocol=udp port=123 action=accept comment="Allow NTP"');
  lines.push('/ip firewall filter add chain=input protocol=tcp dst-port=22 action=accept comment="Allow SSH"');
  lines.push('/ip firewall filter add chain=input protocol=tcp dst-port=8291 action=accept comment="Allow WinBox"');
  lines.push('/ip firewall filter add chain=input protocol=tcp dst-port=8728 action=accept comment="Allow API"');
  lines.push('/ip firewall filter add chain=input protocol=tcp dst-port=443 action=accept comment="Allow HTTPS"');
  lines.push('/ip firewall filter add chain=input action=drop comment="Drop all other input"');
  lines.push('');
  ...
  lines.push('/ip firewall filter add chain=forward action=fasttrack-connection connection-state=established,related comment="FastTrack established connections"');
  lines.push('');

  // Connection Tracking
  lines.push('# Connection Tracking Settings');
  lines.push('/ip firewall connection tracking set tcp-close-wait-time=10s tcp-time-wait-time=10s');
  lines.push('');
  ...
  if (router.hotspot_enabled) {
    ...
    lines.push('/ip hotspot profile add name=hs-prof dns-name=login.local hotspot-address=' + hotspotGateway + ' login-by=cookie,http-pap');
    lines.push(`/ip hotspot user profile set default idle-timeout=10m keepalive-timeout=2m shared-users=1`);
    lines.push(`/ip hotspot add name=hotspot1 interface=${lanBridge} address-pool=hs_pool profile=hs-prof disabled=no`);
    lines.push(`/ip address add address=${hotspotIp} interface=${lanBridge} comment="Hotspot Gateway"`);
    lines.push(`/ip dhcp-server network add address=${hotspotNetwork} gateway=${hotspotGateway} dns-server=${hotspotGateway}`);
    lines.push('');
  }
  ...
  if (router.bandwidth_control) {
    ...
    lines.push('/queue simple add name=default-limit target=192.168.88.0/24 max-limit=10M/10M comment="Default bandwidth limit"');
    lines.push('');
  }
  ...
  if (router.vlans && router.vlans.length > 0) {
    ...
    router.vlans.forEach(vlan => {
      lines.push(`/interface vlan add name=vlan${vlan.id} vlan-id=${vlan.id} interface=${lanBridge} comment="${vlan.name || 'VLAN ' + vlan.id}"`);
      if (vlan.ip) {
        lines.push(`/ip address add address=${vlan.ip} interface=vlan${vlan.id} comment="VLAN ${vlan.id} IP"`);
      }
    });
    lines.push('');
  }
  ...
  if (router.static_routes && router.static_routes.length > 0) {
    ...
    router.static_routes.forEach(route => {
      lines.push(`/ip route add dst-address=${route.dst} gateway=${route.gateway} comment="${route.comment || 'Static route'}"`);
    });
    lines.push('');
  }
  ...
  if (router.snmp_enabled) {
    ...
    lines.push('/snmp set enabled=yes contact="' + (router.snmp_contact || 'admin@local') + '" location="' + (router.snmp_location || 'Main Office') + '"');
    if (router.snmp_community) {
      lines.push(`/snmp community set 0 name="${router.snmp_community}" read-access=yes write-access=no`);
    }
    lines.push('');
  }
  ...
  lines.push('/log info message="MikroTik Router provisioned successfully by billing system"');
  lines.push('');
