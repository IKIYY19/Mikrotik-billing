...
function getEncryptionKey() {
  return Buffer.from((process.env.ENCRYPTION_KEY || 'default-key-change-in-production-32').slice(0, 32));
}
...
function buildConnectionFields(router, overrides = {}) {
  const connectionType = overrides.connection_type || router.connection_type || 'api';
  const managementPort = Number(overrides.mgmt_port || router.mgmt_port || 8728);
  ...
  const passwordEncrypted = overrides.mgmt_password
    ...
    : (overrides.mgmt_password_encrypted || router.mgmt_password_encrypted || null);
...
async function ensureMikrotikConnection(routerId, overrides = {}) {
  ...
  if (!fields.ip_address) {
    return { success: false, status: 'skipped', error: 'Router IP address is missing' };
  }
  if (!fields.username || !fields.password_encrypted) {
    return { success: false, status: 'skipped', error: 'Management credentials are missing on this zero-touch router' };
  }
...
async function syncRouterSubscriptions(router, connection) {
  ...
  const targets = subscriptions.filter((subscription) =>
    subscription.router_id === router.id || subscription.mikrotik_connection_id === connection.id
  );
  ...
  for (const subscription of targets) {
    ...
    const expanded = {
      ...updated,
      customer: updated.customer || await billing.getCustomerById(updated.customer_id),
    ...
    if (connection.connection_type !== 'api') {
      results.push({
        ...
        success: false,
        status: 'skipped',
        message: 'Billing sync currently requires an API-backed MikroTik connection',
      });
      continue;
    }

    try {
      const syncResult = await mikrotikProvisioning.reconcileSubscription(expanded);
...
async function activateRouterInBilling(routerId, overrides = {}) {
  try {
    ...
    if (!linkResult.success) {
      await updateRouterLink(routerId, currentRouter?.linked_mikrotik_connection_id || null, linkResult.error);
      return linkResult;
    }

    const syncResults = await syncRouterSubscriptions(linkResult.router, linkResult.connection);
    ...
    const currentRouter = await getRouterById(routerId);
    await updateRouterLink(routerId, currentRouter?.linked_mikrotik_connection_id || null, error.message);
    return {
      success: false,
      status: 'failed',
      error: error.message,
