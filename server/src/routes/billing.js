...
// ═══════════════════════════════════════════════════════════════════════════
// PRODUCTION BILLING — WEEK 1 & 2 FEATURES
// ═══════════════════════════════════════════════════════════════════════════

// ─── PDF Invoice Download ────────────────────────────────────────────────────
router.get('/invoices/:id/pdf', async (req, res) => {
  try {
    const invoice = await billing.getInvoiceById(req.params.id);
    if (!invoice) { return res.status(404).json({ error: 'Invoice not found' }); }

    const { generateInvoiceHTML, getCompanySettings } = require('../services/pdfService');
...
router.get('/customers/:id/statement', async (req, res) => {
  try {
    const customer = await billing.getCustomerById(req.params.id);
    if (!customer) { return res.status(404).json({ error: 'Customer not found' }); }
      ...
      const [invRes, payRes] = await Promise.all([
        ...
        currentDb.query(
          `SELECT p.*, i.invoice_number FROM payments p
           ...
           WHERE p.customer_id = $1 AND p.received_at >= NOW() - INTERVAL '12 months'
    ...
    const { generateStatementHTML, getCompanySettings } = require('../services/pdfService');
    const company = await getCompanySettings();
    const html = generateStatementHTML(customer, invoices, payments, company);

    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Content-Disposition', `inline; filename="Statement-${customer.name?.replace(/\s+/g,'_')}.html"`);
    res.send(html);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
router.get('/tax-rates', async (req, res) => {
  try {
    const currentDb = global.dbAvailable ? global.db : null;
    if (!currentDb) { return res.json([{ id: 'default', name: 'VAT', rate: 16, is_default: true, is_active: true }]); }
    const result = await currentDb.query('SELECT * FROM tax_rates ORDER BY is_default DESC, name ASC');
    res.json(result.rows);
...
router.post('/tax-rates', async (req, res) => {
  try {
    const currentDb = global.dbAvailable ? global.db : null;
    if (!currentDb) { return res.status(503).json({ error: 'Database required' }); }
    const { name, rate, description, is_default } = req.body;
    if (is_default) {
      await currentDb.query('UPDATE tax_rates SET is_default = false');
    }
    const { v4: uuid } = require('uuid');
    const result = await currentDb.query(
      `INSERT INTO tax_rates (id, name, rate, description, is_default, is_active)
       VALUES ($1,$2,$3,$4,$5,true) RETURNING *`,
      [uuid(), name, parseFloat(rate) || 0, description || '', Boolean(is_default)]
    );
    res.status(201).json(result.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/tax-rates/:id', async (req, res) => {
  try {
    ...
    const result = await currentDb.query(
      `UPDATE tax_rates SET name=COALESCE($1,name), rate=COALESCE($2,rate),
       description=COALESCE($3,description), is_active=COALESCE($4,is_active)
       WHERE id=$5 RETURNING *`,
      [name, rate ? parseFloat(rate) : null, description, is_active, req.params.id]
    );
...
router.put('/tax-rates/:id/default', async (req, res) => {
  try {
    const currentDb = global.dbAvailable ? global.db : null;
    if (!currentDb) { return res.status(503).json({ error: 'Database required' }); }
    await currentDb.query('UPDATE tax_rates SET is_default = false');
    const result = await currentDb.query(
      'UPDATE tax_rates SET is_default = true WHERE id = $1 RETURNING *',
      [req.params.id]
    );
    if (!result.rows.length) { return res.status(404).json({ error: 'Tax rate not found' }); }
    res.json(result.rows[0]);
...
router.delete('/tax-rates/:id', async (req, res) => {
  try {
    const currentDb = global.dbAvailable ? global.db : null;
    if (!currentDb) { return res.status(503).json({ error: 'Database required' }); }
    await currentDb.query('DELETE FROM tax_rates WHERE id = $1', [req.params.id]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ─── Recurring Billing — Manual Trigger ──────────────────────────────────────
router.post('/invoices/generate-monthly', async (req, res) => {
  try {
    const created = await billing.generateMonthlyInvoices();
    res.json({ success: true, created: created.length, invoices: created });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

router.post('/invoices/generate-recurring', async (req, res) => {
  try {
    const { runRecurringBilling } = require('../cron/recurringBilling');
    const results = await runRecurringBilling({ force: Boolean(req.body.force) });
...
router.post('/invoices/bulk-generate', async (req, res) => {
  try {
    const { customer_ids = [], use_plan_price = true, amount: fixedAmount, due_date, notes } = req.body;
    ...
    const dueDateStr = due_date || new Date(now.getFullYear(), now.getMonth(), 15).toISOString().split('T')[0];
    const currentDb = global.dbAvailable ? global.db : null;
    ...
    for (const customerId of customer_ids) {
      try {
        // Check existing invoice this month
        if (currentDb) {
          const existing = await currentDb.query(
            `SELECT id FROM invoices WHERE customer_id = $1
             AND EXTRACT(MONTH FROM created_at) = $2 AND EXTRACT(YEAR FROM created_at) = $3 LIMIT 1`,
            [customerId, currentMonth, currentYear]
          );
          if (existing.rows.length > 0) {
            results.skipped.push({ customer_id: customerId, reason: 'Invoice already generated this month' });
            continue;
          }
        }

        let amount = fixedAmount ? parseFloat(fixedAmount) : 0;
        let invoiceNotes = notes || 'Monthly internet service';

        if (use_plan_price && !fixedAmount) {
          const subs = await billing.listSubscriptions();
          const activeSub = subs.find(s => s.customer_id === customerId && s.status === 'active');
          if (activeSub) {
            amount = activeSub.plan?.price || activeSub.plan_price || 0;
            const planName = activeSub.plan?.name || 'Internet Service';
            invoiceNotes = notes || `Monthly internet service — ${planName}`;
          }
        }

        if (!amount || amount <= 0) {
          results.skipped.push({ customer_id: customerId, reason: 'No plan price found' });
          continue;
        }
        ...
        results.created.push(invoice);
      } catch (err) {
        results.errors.push({ customer_id: customerId, error: err.message });
      }
    }
...
router.get('/reports/aging', async (req, res) => {
  try {
    const currentDb = global.dbAvailable ? global.db : null;
    if (!currentDb) { return res.status(503).json({ error: 'Database required for aging report' }); }
    ...
    const result = await currentDb.query(`
      SELECT
        ...
        COALESCE(SUM(CASE WHEN (CURRENT_DATE - i.due_date::date) BETWEEN 1 AND 30
          THEN (i.total - COALESCE(paid.paid_amount,0)) ELSE 0 END), 0) as days_0_30,
        COALESCE(SUM(CASE WHEN (CURRENT_DATE - i.due_date::date) BETWEEN 31 AND 60
          THEN (i.total - COALESCE(paid.paid_amount,0)) ELSE 0 END), 0) as days_31_60,
        COALESCE(SUM(CASE WHEN (CURRENT_DATE - i.due_date::date) BETWEEN 61 AND 90
          THEN (i.total - COALESCE(paid.paid_amount,0)) ELSE 0 END), 0) as days_61_90,
        COALESCE(SUM(CASE WHEN (CURRENT_DATE - i.due_date::date) > 90
          THEN (i.total - COALESCE(paid.paid_amount,0)) ELSE 0 END), 0) as days_over_90,
        COALESCE(SUM(i.total - COALESCE(paid.paid_amount,0)), 0) as total_outstanding
      FROM invoices i
      ...
      HAVING COALESCE(SUM(i.total - COALESCE(paid.paid_amount,0)), 0) > 0
      ORDER BY total_outstanding DESC
    `);

    const customers = result.rows;
    const summary = {
      total_0_30: customers.reduce((s, r) => s + parseFloat(r.days_0_30), 0),
      total_31_60: customers.reduce((s, r) => s + parseFloat(r.days_31_60), 0),
      total_61_90: customers.reduce((s, r) => s + parseFloat(r.days_61_90), 0),
      total_over_90: customers.reduce((s, r) => s + parseFloat(r.days_over_90), 0),
      grand_total: customers.reduce((s, r) => s + parseFloat(r.total_outstanding), 0),
...
// ─── Late Penalties — Manual Trigger ─────────────────────────────────────────
router.post('/reports/apply-late-penalties', async (req, res) => {
  try {
    const { runLatePenalties } = require('../cron/latePenalties');
    const results = await runLatePenalties();
    res.json({ success: true, ...results });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
