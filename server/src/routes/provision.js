...
router.get("/enroll/bootstrap/:token", async (req, res) => {
  try {
    ...
    const script = [
      ...
      '  :set ethList ($ethList . (($ethCount > 1) ? "," : "") . $n)',
      "}",
      ':put "[ZTP] ================================"',
      ':put ""',
      "",
      ...
      ":if ([:len $wanPort] = 0) do={",
      "  :local firstEth [/interface ethernet get ([/interface ethernet find]->0) name]",
      "  :set wanPort $firstEth",
      '  :put "[ZTP] Auto-detected WAN: $wanPort"',
      "}",
      ...
      `:local autoUrl ($serverUrl . "/mikrotik/enroll/auto-complete/" . $enrollToken . "?serverUrl=" . [$ztpUrlEncode $serverUrl] . "&wan=" . [$ztpUrlEncode $wanPort] . "&lan=" . [$ztpUrlEncode $lanPorts])`,
      ...
      ':put "[ZTP] Done. Router provisioned — WAN=" . $wanPort . " LAN=" . $lanPorts',
      "#############################################",
...
router.get("/ztp/one-shot/:token", async (req, res) => {
  try {
    ...
    const script = [
      ...
      "",
      ':put "[ZTP] One-shot provisioning complete! Router configured — WAN=" . $wanPort . " LAN=" . $lanPorts',
