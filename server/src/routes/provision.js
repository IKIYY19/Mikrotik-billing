...
router.get("/enroll/bootstrap/:token", async (req, res) => {
  try {
    ...
    const script = [
      ...
      "  :while ($i < $len) do={",
      "    :set ch [:pick $str $i]",
      '    :if ($ch = " ") do={ :set result ($result . "%20") }',
      '    else={ :if ($ch = "&") do={ :set result ($result . "%26") }',
      '    else={ :if ($ch = "=") do={ :set result ($result . "%3D") }',
      '    else={ :if ($ch = "/") do={ :set result ($result . "%2F") }',
      '    else={ :if ($ch = "?") do={ :set result ($result . "%3F") }',
      '    else={ :if ($ch = "#") do={ :set result ($result . "%23") }',
      '    else={ :if ($ch = "+") do={ :set result ($result . "%2B") }',
      '    else={ :if ($ch = "@") do={ :set result ($result . "%40") }',
      '    else={ :if ($ch = ":") do={ :set result ($result . "%3A") }',
