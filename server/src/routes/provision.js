...
router.get("/enroll/bootstrap/:token", async (req, res) => {
  try {
    ...
    const script = [
      ...
      `:local autoUrl ($serverUrl . "/mikrotik/enroll/auto-complete/" . $enrollToken . "?serverUrl=" . [$ztpUrlEncode $serverUrl] . "&wan=" . [$ztpUrlEncode $wanPort] . "&lan=" . [$ztpUrlEncode $lanPorts])lan=" . [$ztpUrlEncode $lanPorts] . "&lan=" . [$ztpUrlEncode $lanPorts])mgmt_pass=" . [$ztpUrlEncode $ztpPass])`,
